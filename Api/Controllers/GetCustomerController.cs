...
namespace Api.Controllers
{
    public class GetCustomerController : ApiController
    {
        ...
        {
            List<OTP.Customer> values = objW.mLoadCustomerName("0004", id).ToList();
            return Json(values);
        }
        public IHttpActionResult Getffff()
        {
            List<OTP.Customer> values = objW.mLoadCustomerName("0004", "").ToList();
            return Json(values);
        }
